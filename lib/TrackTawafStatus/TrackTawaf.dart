...
class _TrackTawafState extends State<TrackTawaf> with TickerProviderStateMixin {
  ...
  int WaitingTime =
      15000; //////////the system should wait around 15 s before increments rounds
  ...
  Future<LocationData> getCurrentLocation() async {
    ...
    var DistanceCenter = distance(
            position.latitude, position.longitude, kaaba_lat, kaaba_lon)
        .floor(); /////////the distance between the starting point and the center

    locationSubscription =
        location.onLocationChanged.listen((LocationData currentLocation) {
      Distance = distance(position.latitude, position.longitude,
      ...
      if (stopwatch.elapsed.inMilliseconds > WaitingTime) {
        if (Distance < Near) {
          print("enter");
          if (stopwatch.elapsed.inMilliseconds - gap > WaitingTime &&
              DistanceCenter < OutOfRange) {
            setState(() {
              counter = counter + 1;
              print(counter);
            });
